...
namespace Tests
{
    ...
    {
        public static bool EnoughIngredients(IDictionary<string, int> recipeIngredients, 
            IDictionary<string, int> stock)
        {
            return recipeIngredients.All(ingr =>
            ingr.Value == 0 ? true :
            stock.ContainsKey(ingr.Key) ? 
            stock[ingr.Key] >= ingr.Value : false);
        }

        public static IEnumerable<string> CookableRecipes(
            IDictionary<string, IDictionary<string, int>> recipes, 
            IDictionary<string, int> stock)
        {
            return recipes.Where(r => EnoughIngredients(r.Value, stock))
                .Select(r => r.Key).OrderBy(r => r);
        }

        public static int DishCount(IDictionary<string, int> recipe, 
            IDictionary<string, int> stock)
        {
            return EnoughIngredients(recipe, stock) ? 
                recipe.Select(r => stock[r.Key] / r.Value).Min() : 0;
        }

        public static int RecipeCost(IDictionary<string, int> recipe, 
            IDictionary<string, int> ingredientCosts)
        {
            return recipe.Select(r => ingredientCosts[r.Key] * r.Value).Sum();
        }

        public static string MostExpensiveRecipe(
            IDictionary<string, IDictionary<string, int>> recipes, 
            IDictionary<string, int> ingredientCosts)
        {
            return recipes.ToDictionary(r => r.Key, r => 
            RecipeCost(r.Value, ingredientCosts)).OrderByDescending(r => r.Value).First().Key;
        }

        public static string MostUsedIngredient(
            IDictionary<string, IDictionary<string, int>> recipes)
        {
            return recipes.Select(r => r.Value.Select(r2 => r2.Key))
                .SelectMany(i => i).GroupBy(i => i, i => 1,
                    (i2, list) => new {Key=i2, Value=list.Count() }
                ).OrderByDescending(r => r.Value).First().Key;
        }

        public static IEnumerable<string> UselessIngredients(
            IDictionary<string, IDictionary<string, int>> recipes, 
            IDictionary<string, int> stock)
        {
            return recipes.Count() != 0 ? stock.Where(s => 
                !recipes.Select(r => r.Value.Select(r2 => r2.Key)).
                SelectMany(i => i).Contains(s.Key)
            ).Select(s => s.Key).OrderBy(r => r) 
            : stock.Select(s => s.Key).OrderBy(r => r);
        }

        public static IEnumerable<string> VegetarianRecipes(
            IDictionary<string, IDictionary<string, int>> recipes, 
            IList<string> veg)
        {
            return recipes.Where(r => 
            r.Value.Keys.All(i => veg.Contains(i)) 
            ).Select(r => r.Key).OrderBy(r => r);
        }
    }
